...
type AISearchLanguage = 'en' | 'cn';

const translations = {
  en: {
    title: 'AI Chat',
    description: 'AI can be inaccurate, please verify the answers.',
    close: 'Close',
    retry: 'Retry',
    clear: 'Clear Chat',
    placeholder: 'Ask a question',
    loadingPlaceholder: 'AI is answering...',
    abort: 'Abort Answer',
  ...
  cn: {
    title: 'AI 助手',
    description: 'AI 可能出错，请核对重要信息。',
    close: '关闭',
    retry: '重新生成',
    clear: '清空对话',
    placeholder: '输入你的问题',
    loadingPlaceholder: 'AI 正在回答...',
    abort: '停止回答',
    empty: '在下方开始提问',
    roles: {
      user: '你',
    ...
    searchFailed: '搜索失败',
    searching: '搜索中...',
    searchResults: (count: number) => `找到 ${count} 条相关结果`,
    requestFailed: '请求失败',
  },
} satisfies Record<AISearchLanguage, Record<string, unknown>>;
...
export function AISearchPanelHeader({ className, ...props }: ComponentProps<'div'>) {
  const { setOpen, language } = useAISearchContext();
  ...
  return (
    ...
    >
      <div className="px-3 py-2 flex-1">
        ...
        <p className="text-xs text-fd-muted-foreground">{t.description}</p>
      </div>
...
export function AISearchInput(props: ComponentProps<'form'>) {
  ...
  return (
    <form {...props} className={cn('flex items-start pe-2', props.className)} onSubmit={onStart}>
      <Input
        value={input}
        placeholder={isLoading ? t.loadingPlaceholder : t.placeholder}
        autoFocus
        className="p-3"
...
function Message({ message, ...props }: { message: ChatUIMessage } & ComponentProps<'div'>) {
  ...
  return (
    <div onClick={(e) => e.stopPropagation()} {...props}>
      ...
      >
        {t.roles[message.role as keyof typeof t.roles] ?? t.roles.unknown}
      </p>
        ...
        return (
          ...
          >
            ...
            {call.state === 'output-error' || call.state === 'output-denied' ? (
              <p className="text-fd-error">{call.errorText ?? t.searchFailed}</p>
            ) : (
              <p>{!call.output ? t.searching : t.searchResults(call.output.length)}</p>
            )}
          </div>
        );
      })}
    ...
    >
      {messages.length === 0 ? (
        <div className="text-sm text-fd-muted-foreground/80 size-full flex flex-col items-center justify-center text-center gap-2">
          ...
          <p onClick={(e) => e.stopPropagation()}>{t.empty}</p>
        </div>
      ) : (
          ...
          {chat.error && (
            <div className="p-2 bg-fd-secondary text-fd-secondary-foreground border rounded-lg">
              <p className="text-xs text-fd-muted-foreground mb-1">
                {t.requestFailed}: {chat.error.name}
              </p>
