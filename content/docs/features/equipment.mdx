---
title: "Equipment"
description: "Show devices, software, desks, or tools you use regularly."
---
## Purpose

Show devices, software, desks, or tools you use regularly.

## Create the page

Create a page under `source/kit/index.md` and set a clear title.

```md
---
title: Equipment
date: 2026-01-01
type: kit
---
```

## Create the data file

Put repeatable content in `source/_data/kit.yml`.

```yaml
items:
  - name: Example item
    description: Replace this with your own content.
    link: https://example.com
```

## Theme configuration

Enable the page module in the theme configuration when the feature requires a switch.

```yaml
pages:
  kit:
    enable: true
    data: kit.yml
```

## File structure

<Files>
  <File name="Blog Directory" />
  <File name="source" />
  <File name="_data" />
  <File name="kit.yml" />
  <File name="kit" />
  <File name="index.md" />
</Files>

## Field reference

- `name`: display name.
- `description`: short explanatory text.
- `link`: optional external or internal URL.
- `cover`: optional image path when the page layout supports covers.
- `tags`: optional labels used for grouping or filtering.

<Callout type="success">
Start with one or two items, confirm the page renders, then expand the data file.
</Callout>
