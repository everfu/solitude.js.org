...
---
If a problem began while updating a 3.x blog, complete [Upgrade to 4.0](/docs/getting-started/upgrade-v4) before isolating individual modules.
...
Navigate repeatedly between the same two pages. Duplicate dialogs, players, callbacks, or network requests usually indicate custom code that registers permanent resources on every `afterNavigate`. Subscribe once, then use the page-scoped cleanup methods in [Browser API](/docs/configuration/browser-api).
