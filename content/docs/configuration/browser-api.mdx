...
icon: "FileText"
---
Solitude 4 exposes its public browser integration surface through `window.Solitude`. Feature-specific objects and implementation details are not public contracts. If you are updating custom code from 3.x, begin with [Upgrade to 4.0](/docs/getting-started/upgrade-v4), then use the legacy-call map below.
...
This table covers JavaScript calls only. Use the complete [Upgrade to 4.0](/docs/getting-started/upgrade-v4) checklist for theme files, configuration, CDN assets, and regression testing.
...
Do not integrate against `sco`, `rm`, `ai`, or `coverColor`. For CDN-hosted or self-hosted Solitude JavaScript, publish the complete same-version `source/js/` tree so relative ES-module imports continue to resolve. See [Troubleshooting](/docs/faq/troubleshooting) for module-loading and PJAX checks.
